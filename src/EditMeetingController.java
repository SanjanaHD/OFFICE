...
public class EditMeetingController {
    @FXML TableView<MeetingRow> meetingsTable = new TableView<>();

    @FXML TableColumn<MeetingRow, LocalDate> dateColumn = new TableColumn();

    @FXML TableColumn<MeetingRow, String> topicColumn = new TableColumn();

    @FXML Button editMeetingBtn = new Button();

    String selectedMeetingId;

    public void initialize() throws UnknownHostException{
        populateTable();
        editMeetingBtn.setDisable(true);

        meetingsTable.setOnMouseClicked(event -> {
            editMeetingBtn.setDisable(false);
    ...
    public void editMeeting() throws IOException{
        Stage editMeetingStage = new Stage();
        FXMLLoader loader = new FXMLLoader(getClass().getResource("EditSelectedMeeting.fxml"));
        Scene scene = new Scene(loader.load());

        EditSelectedMeetingController controller = loader.getController();
        controller.initData(selectedMeetingId);
        //controller.setId(selectedMeetingId);

        editMeetingStage.setTitle("Edit Meeting");
        editMeetingStage.setScene(scene);
        editMeetingStage.setResizable(false);

        editMeetingStage.setOnCloseRequest(new EventHandler<WindowEvent>() {
            @Override
